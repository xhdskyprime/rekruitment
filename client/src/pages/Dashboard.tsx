...
const Dashboard = () => {
  ...
  const [newPositionCode, setNewPositionCode] = useState<string>('');
  ...
  const handleAddPosition = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rawCode = newPositionCode.replace(/\D/g, '');
      const code = rawCode.padStart(2, '0').slice(-2);
      if (!name || !code) return;
      if (editingPosition) {
        await axios.put(`/admin/positions/${editingPosition.id}`, { name, code }, { withCredentials: true });
        setEditingPosition(null);
        Swal.fire('Berhasil', 'Posisi berhasil diperbarui', 'success');
      } else {
        await axios.post('/admin/positions', { name, code }, { withCredentials: true });
  ...
  const openPreview = (applicant: Applicant, type: string, label: string, path: string, status: string) => {
    ...
    if (path && path.includes('/file/proxy/')) {
      ...
      axios.get(`/file/meta/${id}`, { withCredentials: true })
        .then(res => {
          if (res.data?.mimeType) {
            setPreviewMime(res.data.mimeType);
          }
        })
        .catch(() => {});
    }
    ...
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <aside className={`bg-indigo-700 flex flex-col fixed md:relative z-30 h-full transition-all duration-300 w-64 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0 md:w-20'}`}>
        ...
        <nav className="flex-1 py-4 space-y-1 overflow-y-auto overflow-x-hidden no-scrollbar">
          {isSidebarOpen ? (
            <div className="px-6 py-2 text-[10px] font-bold text-white/40 uppercase tracking-widest">Menu</div>
          ) : (
            <div className="h-4" />
          )}
          <button
          ...
          {currentUserRole === 'superadmin' && (
            <>
              {isSidebarOpen ? (
                ...
                <>
                  <div className="mt-4 mb-2 h-px bg-white/10 mx-4"></div>
                  <button
                    onClick={() => setActiveTab('positions')}
          ...
          {currentUserRole === 'superadmin' && (
            <>
              <div className={`px-6 pt-6 pb-2 text-[10px] font-black text-white/30 uppercase tracking-[0.2em] transition-opacity duration-300 ${!isSidebarOpen && 'opacity-0 h-0 p-0 overflow-hidden'}`}>
                PENGATURAN
              </div>
              <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative bg-[#F3F4F6]">
        {/* Header */}
        <header className="h-28 flex items-center justify-between px-12 z-10 sticky top-0 bg-[#F3F4F6]/80 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4">
             <div className="flex items-center gap-4 bg-white p-2.5 pr-8 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-white hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-all duration-300 group cursor-pointer">
                <div className="h-11 w-11 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-black text-xl shadow-lg shadow-indigo-200 group-hover:scale-105 transition-transform duration-300">
                   {currentUsername ? currentUsername.charAt(0).toUpperCase() : <User size={22} />}
                </div>
                <div className="text-left hidden sm:block">
                   <p className="text-sm font-black text-gray-900 leading-none mb-1.5">{currentUsername || 'Admin'}</p>
                   <div className="flex items-center">
                     <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-2 animate-pulse"></div>
                     <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">{currentUserRole}</p>
                   </div>
                </div>
             </div>
             ...
             <button 
               ...
               title="Keluar"
               className="h-14 w-14 flex items-center justify-center bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-white text-rose-500 hover:text-rose-600 hover:shadow-[0_8px_30px_rgba(244,63,94,0.15)] hover:scale-105 transition-all duration-300 group"
             >
        ...
        <main className="flex-1 overflow-y-auto p-10 pt-2">
          <div key={activeTab} className="max-w-7xl mx-auto content-transition">
            {/* Stats Cards - Only show on Applicants or Verification tab */}
            {(activeTab === 'applicants' || activeTab === 'verification') && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-10">
                <div className="bg-white p-8 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex items-center hover:shadow-[0_20px_40px_rgba(79,70,229,0.1)] transition-all duration-500 group relative overflow-hidden">
                  ...
                  <div className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl mr-5 group-hover:scale-110 transition-transform duration-300 relative z-10">
                    <Users className="w-7 h-7" />
                  </div>
                  <div className="relative z-10">
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-1">Total Pelamar</p>
                    <p className="text-3xl font-black text-gray-900">{stats.total}</p>
                  </div>
                </div>
                <div className="bg-white p-8 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex items-center hover:shadow-[0_20px_40px_rgba(245,158,11,0.1)] transition-all duration-500 group relative overflow-hidden">
                  ...
                  <div className="p-4 bg-amber-50 text-amber-500 rounded-2xl mr-5 group-hover:scale-110 transition-transform duration-300 relative z-10">
                    <Clock className="w-7 h-7" />
                  </div>
                  <div className="relative z-10">
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-1">Menunggu</p>
                    <p className="text-3xl font-black text-gray-900">{stats.pending}</p>
                  </div>
                </div>
                <div className="bg-white p-8 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex items-center hover:shadow-[0_20px_40px_rgba(16,185,129,0.1)] transition-all duration-500 group relative overflow-hidden">
                  ...
                  <div className="p-4 bg-emerald-50 text-emerald-500 rounded-2xl mr-5 group-hover:scale-110 transition-transform duration-300 relative z-10">
                    <CheckCircle className="w-7 h-7" />
                  </div>
                  <div className="relative z-10">
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-1">Lolos</p>
                    <p className="text-3xl font-black text-gray-900">{stats.verified}</p>
                  </div>
                </div>
                <div className="bg-white p-8 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white flex items-center hover:shadow-[0_20px_40px_rgba(244,63,94,0.1)] transition-all duration-500 group relative overflow-hidden">
                  ...
                  <div className="p-4 bg-rose-50 text-rose-500 rounded-2xl mr-5 group-hover:scale-110 transition-transform duration-300 relative z-10">
                    <XCircle className="w-7 h-7" />
                  </div>
                  <div className="relative z-10">
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-1">Ditolak</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-[#F8F9FB] text-gray-500 text-[11px] uppercase font-bold tracking-widest border-b border-gray-100">
                <tr>
                  {activeTab === 'applicants' ? (
                    <>
                      ...
                      <th className="px-6 py-5">Tempat / Tanggal Lahir</th>
                      <th className="px-6 py-5">Umur</th>
                      <th className="px-6 py-5">Pendidikan</th>
                      <th className="px-6 py-5">Institusi</th>
                      <th className="px-6 py-5">Jurusan</th>
                      <th className="px-6 py-5">IPK</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  <tr><td colSpan={12} className="px-8 py-12 text-center text-gray-400 font-medium">Loading data...</td></tr>
                ) : applicants.length === 0 ? (
                  <tr><td colSpan={12} className="px-8 py-12 text-center text-gray-400 font-medium">Tidak ada data pelamar.</td></tr>
                ) : (
                  applicants.map((app) => (
                    <tr key={app.id} className="hover:bg-gray-50/50 transition-colors duration-200">
                      {activeTab === 'applicants' ? (
                        <>
                          ...
                          <td className="px-6 py-5 text-gray-600 font-bold text-sm">
                            {calculateAgeAtRegistration(app.birthDate, app.createdAt)}
                          </td>
                          <td className="px-6 py-5 text-gray-600 text-sm">{app.education}</td>
                          <td className="px-6 py-5 text-gray-600 text-sm">{app.institution || '-'}</td>
                          <td className="px-6 py-5 text-gray-600 text-sm">{app.major || '-'}</td>
                          <td className="px-6 py-5 text-gray-600 font-bold text-sm">{app.gpa ? app.gpa.toFixed(2) : '-'}</td>
                          <td className="px-6 py-5">
                            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[11px] font-bold border border-blue-100 whitespace-nowrap">
                              {app.position}
                            </span>
                          </td>
                          <td className="px-6 py-5 text-gray-500 text-sm">{app.email}</td>
                          <td className="px-6 py-5 text-gray-500 text-sm">{app.phoneNumber || '-'}</td>
                        </>
                      ) : (
                        <>
                          <td className="px-8 py-5">
                            <div className="flex items-center">
                              {app.pasFotoPath ? (
                                <div className="h-10 w-10 flex-shrink-0 mr-4 cursor-pointer ring-2 ring-white shadow-sm rounded-full overflow-hidden" onClick={() => window.open(`${app.pasFotoPath}`, '_blank')}>
                                  <img className="h-full w-full object-cover" src={`${app.pasFotoPath}`} alt="" />
                                </div>
                              ) : (
                                <div className="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center mr-4 text-gray-400">
                                  <User size={20} />
                                </div>
                              )}
                              <div>
                          ...
                          <td className="px-6 py-5">
                            ...
                            {app.status === 'rejected' && (
                              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold bg-rose-100 text-rose-700 border border-rose-200 uppercase tracking-wider">
                                <XCircle className="w-3 h-3 mr-1"/> Ditolak
                              </span>
                            )}
                            {app.status === 'pending' && (
                              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200 uppercase tracking-wider">
                          ...
                          <td className="px-8 py-5">
                            <div className="flex items-center gap-3">
                              <button
                                  ...
                                  className="flex items-center px-3 py-2 bg-indigo-600 text-white text-[11px] font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-sm hover:shadow-indigo-200 hover:-translate-y-0.5"
                              ...
                              {app.status === 'verified' && (
                                <button
                                  ...
                                  className="flex items-center px-3 py-2 bg-tangerang-purple text-white text-[11px] font-bold rounded-xl hover:bg-tangerang-dark transition-all shadow-sm hover:shadow-purple-200 hover:-translate-y-0.5"
                              ...
                              {currentUserRole === 'superadmin' && (
                                <button
                                  ...
                                  className="text-rose-400 hover:text-rose-600 p-2 rounded-xl hover:bg-rose-50 transition-colors"
                                  title="Hapus Pelamar"
                                >
                                  <Trash2 className="w-4 h-4" />
                                </button>
          ...
          <div className="px-8 py-6 border-t border-gray-50 flex items-center justify-between bg-[#F8F9FB]/50">
            <div className="text-xs text-gray-400 font-bold uppercase tracking-wider">
              Hal {pagination.currentPage} dari {pagination.totalPages} <span className="mx-2 text-gray-200">|</span> {pagination.totalItems} total data
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => setPage(prev => Math.max(prev - 1, 1))}
                disabled={pagination.currentPage === 1}
                className="px-4 py-2 bg-white border border-gray-100 rounded-xl text-xs font-bold text-gray-600 disabled:opacity-50 hover:bg-gray-50 transition-all shadow-sm disabled:shadow-none"
              >
                Sebelumnya
              </button>
              <button
      ...
      {previewFile && selectedApplicant && (
        <div className={`fixed inset-0 bg-black z-50 flex items-center justify-center animate-in fade-in duration-200 ${isFullscreen ? '' : 'bg-opacity-75 p-4 backdrop-blur-sm'}`}>
          <div className={`bg-white shadow-2xl overflow-hidden flex flex-col ${isFullscreen ? 'w-full h-full' : 'rounded-2xl w-full max-w-5xl h-[95vh]'}`}>
            {!isFullscreen && (
            ...
            <div className={`flex-1 overflow-hidden relative flex items-center justify-center ${isFullscreen ? 'p-0 bg-gray-900' : 'p-4 bg-gray-100'}`}>
              {isFullscreen && (
                <button 
                  onClick={() => setIsFullscreen(false)}
                  className="absolute bottom-8 right-8 z-50 p-4 bg-white/5 backdrop-blur-xl text-white hover:bg-white/15 hover:scale-110 active:scale-95 rounded-full transition-all duration-300 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] flex items-center justify-center border border-white/10 group"
                  title="Keluar Layar Penuh"
                >
                  <Minimize2 size={24} className="group-hover:rotate-90 transition-transform duration-500" />
                </button>
              )}
              {!previewMime && !previewFile.url.includes('/file/proxy/') && (
                <div className="text-gray-400 flex flex-col items-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-tangerang-purple mb-2"></div>
